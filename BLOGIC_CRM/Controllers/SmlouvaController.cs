    ...
    public async Task<IActionResult> Create(SmlouvaCreateViewModel viewModel)
    {
        ...
        if (ModelState.IsValid)
        {
            var novaSmlouva = new Smlouva
            {
                ...
                SpravceId = viewModel.SpravceId
            };

            var ucastnici = _context.Poradci.Where(p => viewModel.VybraniUcastniciIds.Contains(p.Id)).ToList();

            novaSmlouva.Ucastnici = ucastnici;
