...
public class PoradceController : Controller
{
    ...
    {
        return View(await _context.Poradci.ToListAsync());
    }
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int? id, [Bind("Id,Jmeno,Prijmeni,Email,Telefon,RodneCislo,Vek,SmlouvaPoradce,SpravovaneSmlouvy,CeleJmeno")] Poradce poradce)
    {
        if (id != poradce.Id)
        {
            return NotFound();
